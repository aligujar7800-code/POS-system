...
import { 
  ...
  Package, Check, Users, ShoppingBag, Baby, Tag, RefreshCw 
} from 'lucide-react';
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <div style={{ padding: '14px 20px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', margin: 0 }}>
                  Voucher Items ({cart.length} items, {totalQty} pieces)
                </h3>
              </div>
                ...
                <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#f1f5f9', borderBottom: '2px solid #e2e8f0' }}>
                      <th style={{ padding: '10px 16px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Article</th>
                      <th style={{ padding: '10px 16px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Category</th>
                      <th style={{ padding: '10px 16px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Details</th>
                      <th style={{ padding: '10px 12px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase', width: 80 }}>Qty</th>
                      <th style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase', width: 100 }}>Cost</th>
                      <th style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase', width: 100 }}>Sale</th>
                      <th style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase', width: 110 }}>Total Cost</th>
                      <th style={{ padding: '10px 12px', width: 44 }}></th>
                    </tr>
                  </thead>
                  <tbody>
                    {cart.map((item) => (
                      <tr key={item.cart_id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '12px 16px' }}>
                          <span style={{ fontFamily: 'monospace', fontWeight: 700, color: '#6366f1', fontSize: 13 }}>{item.article_number}</span>
                          <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>{item.product_name}</div>
                        </td>
                        <td style={{ padding: '12px 16px' }}>
                          <span style={{ fontSize: 11, background: '#eef2ff', color: '#6366f1', padding: '3px 8px', borderRadius: 6, fontWeight: 700 }}>{item.main_category}</span>
                          <span style={{ fontSize: 11, background: '#f1f5f9', color: '#475569', padding: '3px 8px', borderRadius: 6, fontWeight: 600, marginLeft: 4 }}>{item.sub_category_name}</span>
                        </td>
                        <td style={{ padding: '12px 16px' }}>
                          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                            {item.color && <span style={{ fontSize: 11, background: '#fef3c7', color: '#92400e', padding: '2px 8px', borderRadius: 6, fontWeight: 600 }}>🎨 {item.color}</span>}
                            {item.size && <span style={{ fontSize: 11, background: '#e0f2fe', color: '#0369a1', padding: '2px 8px', borderRadius: 6, fontWeight: 600 }}>📏 {item.size}</span>}
                          </div>
                        </td>
                        <td style={{ padding: '12px', textAlign: 'center' }}>
                          <input
                            ...
                            style={{ width: 56, textAlign: 'center', padding: '4px 6px', border: '1px solid #e2e8f0', borderRadius: 6, fontWeight: 700, fontSize: 13 }}
                          />
                        </td>
                        <td style={{ padding: '12px', textAlign: 'right' }}>
                          <input
                            ...
                            style={{ width: 72, textAlign: 'right', padding: '4px 6px', border: '1px solid #e2e8f0', borderRadius: 6, fontSize: 12 }}
                          />
                        </td>
                        <td style={{ padding: '12px', textAlign: 'right' }}>
                          <input
                            ...
                            onChange={(e) => updateCartItem(item.cart_id, { sale_price: parseFloat(e.target.value) || 0 })}
                            style={{ width: 72, textAlign: 'right', padding: '4px 6px', border: '1px solid #e2e8f0', borderRadius: 6, fontSize: 12 }}
                          />
                        </td>
                        <td style={{ padding: '12px', textAlign: 'right', fontWeight: 700, color: '#1e293b', fontSize: 13 }}>
                          {formatCurrency(item.quantity * item.cost_price, currency_symbol)}
                        </td>
                        <td style={{ padding: '12px', textAlign: 'center' }}>
                          <button onClick={() => removeCartItem(item.cart_id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', padding: 4 }}>
